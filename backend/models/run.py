...
class User(Base):
    ...
    profile_picture_url: Mapped[str | None] = mapped_column(Text)  # For Google OAuth
    google_id: Mapped[str | None] = mapped_column(String(255), unique=True, nullable=True)  # Google OAuth ID
    territory_color: Mapped[str] = mapped_column(String(7), default="#00FF88", nullable=False)
...
class EmailOTPCode(Base):
    ...
    purpose: Mapped[str] = mapped_column(String(30), nullable=False)  # signup | password_reset
